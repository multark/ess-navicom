...
import { Sidebar, Footer,  Navbar, NavbarLogo, NavbarCollapse, NavbarLink,Tabs, Tab } from 'flowbite-vue';
...
const leaveApplicationField = reactive({
  from_date: null,
  to_date: null,
  leave_type: null,
  employee: null,
...
employee_doc.fetch().then((doc) => {
  localStorage.setItem("employee",doc['Employee'])
  checkinField.employee = doc['Employee']
  checkOutField.employee = doc['Employee']
});
...
const employee_checkin = createListResource({
...
employee_checkin.reload();

const employee_checkins = createResource({
  url: "/api/method/ess.api.employee.get_employee_checkin"
})
employee_checkins.fetch()

//Get Employee Checkin from logged in User
const leave_application = createListResource({
  doctype: "Leave Application",
  fields: ["*"],
  cache: "leave_application",
});
leave_application.reload();

// function showLeaveApplicationDialog(){}
let showLeaveApplicationDialog = ref(false)

function checkin(work_from) {
  // addCheckin("In",work_from,checkinField.latitude,checkinField.longitude)
  checkinField.work_from = work_from
  employee_checkin.insert.submit(checkinField);
  employee_in_office.fetch();
  employee_checkin.reload();
  employee_checkins.fetch();
}
function checkOut(work_from) {
  checkOutField.work_from = work_from
  employee_checkin.insert.submit(checkOutField);
  employee_in_office.fetch();
  employee_checkin.reload();
  employee_checkins.fetch();
}
const checkinDoc = useCheckin()
function addCheckin(log_type,work_from,lattitude,longitude){
  checkinDoc.add(
    log_type,work_from,lattitude,longitude
  )
}
...
function handleSuccess(position) {
  ...
  checkOutField.latitude = position.coords.latitude;
  checkOutField.longitude = position.coords.longitude;
...
<template>
  ...
  <div class="antialiased bg-gray-50 dark:bg-gray-900">
    <main class="p-4 h-auto pt-10">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        ...
        <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex flex-col items-center pb-10 pt-10">
                <img class="w-24 h-24 mb-3 rounded-full shadow-lg" src="https://bootdey.com/img/Content/avatar/avatar7.png" alt="Bonnie image"/>
                <h5 class="mb-1 text-xl font-medium text-gray-900 dark:text-white"> {{ employee_doc.data["Employee Name"] }}</h5>
                <span class="text-sm text-gray-500 dark:text-gray-400">{{employee_doc.data["Name"]}}</span>
                <div class="flex mt-4 space-x-3 md:mt-6">
                    <a v-if="employee_doc.data['Designation']" href="#" class="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">{{
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        >
          <div class="flex items-center justify-center mb-2">
            <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white pt-4">
              Members In Office ( {{ employee_in_office.data.emp_in_office.length }})
            </h5>
          </div>
        ...
        >
          <div class="flex items-center justify-center mb-2">
            <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white pt-4">
              Members Out Of Office  ( {{ employee_in_office.data.emp_out_of_office.length }})
            </h5>
          </div>
          <div class="relative overflow-x-auto h-60 overflow-y-visible">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        >
          <div class="flex items-center justify-center mb-2">
            <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white pt-4">
              Members Absent  ( {{ employee_in_office.data.emp_absent.length }})
            </h5>
          </div>
          <div class="relative overflow-x-auto h-60 overflow-y-visible">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        >
          ...
          <div class="relative overflow-x-auto h-60 overflow-y-visible">
            <Table>
              <table-head>
                <table-head-cell>Leave Type</table-head-cell>
                <table-head-cell>Total</table-head-cell>
                <table-head-cell>Expired</table-head-cell>
                <table-head-cell>Taken</table-head-cell>
                <table-head-cell>Pending Approval</table-head-cell>
                <table-head-cell>Remaining</table-head-cell>
                <table-head-cell></table-head-cell>
              </table-head>
              <table-body>
                <table-row  v-for="(value, name, index) in leave_details.data.leave_allocation">
                  <table-cell>{{ name }}</table-cell>
                  <table-cell>{{leave_details.data.leave_allocation[name]["Total Leaves"] || 0 }}</table-cell>
                  <table-cell>{{leave_details.data.leave_allocation[name]["Expired Leaves"] || 0 }}</table-cell>
                  <table-cell>{{leave_details.data.leave_allocation[name]["Taken Leaves"] || 0 }}</table-cell>
                  <table-cell>{{leave_details.data.leave_allocation[name]["Pending Approval"] || 0 }}</table-cell>
                  <table-cell>{{leave_details.data.leave_allocation[name]["Remaining Leaves"] || 0 }}</table-cell>
                  <table-cell>
                    <Button
                      appearance="warning"
                      @click="showLeaveApplicationDialog = true"
                      :loading="leave_application.loading"
                    >Apply</Button>
                  </table-cell>
                </table-row>
              </table-body>
    ...
    :options="{
      actions: [
        { label: 'Submit', appearance: 'success' },
        { label: 'Cancel' },
      ],
    }"
  ...
  >
    <template #body-title>
      <h2 class="text-3xl font-bold">Leave Application</h2>
    </template>
    <template #body-content>
      <div class="row-auto">
        <Input label="Leave Type" type="select" :options="['India', 'Not India']"  v-model="leaveApplicationField.leave_type"/>
      </div>
      <div class="row-auto">
      ...
      <div class="row-auto">
        ...
        <Input label="Half Day" type="checkbox"  />
      </div>
    </template>
  </Dialog>
