...
@frappe.whitelist()
def get_employee_checkin():
    employee_data = get_employee_from_user()
    if employee_data.get('Employee'):
        return frappe.db.get_list("Employee Checkin",
                           filters=[
                                        ['employee',"=",employee_data['Employee']],['time','between',(frappe.utils.nowdate() , frappe.utils.nowdate())]],
                           fields = ['*'],
                           order_by='time desc',
                           )
    else:
...
def members_in_office_status():
    ...
    for employee in employees:
        emp_checkin = frappe.db.sql("""
                            ...
                            left join
                            `tabEmployee`
                            on `tabEmployee`.name = `tabEmployee Checkin`.employee_name
        ...
        if emp_checkin:
            if emp_checkin[-1]['log_type'] == "IN" and emp_checkin[-1]['work_from']=="Office":
...
@frappe.whitelist()
def add_checkin(log_type,work_from,lattitude=None,longitude=None):
    employee_data = get_employee_from_user()
    if employee_data:
