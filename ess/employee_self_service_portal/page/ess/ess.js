frappe.pages['ess'].on_page_load = function(wrapper) {
    // let redirect = new Redirect(wrapper);
    $(wrapper).bind('show', () => {
        window.location.pathname='/ess'
    });
    console.log(wrapper)
}
...
ESS = Class.extend({
	...
	// if(typeof(dicts['filters'])=="string"){
			// 	raw_filters = JSON.parse(dicts['filters'])
			// }
			// 	let list_filter = []
			...
			// 	let route_options = list_filter.reduce((acc, filter) => {
			// 		return Object.assign(acc, {
			// 			[`${filter[0]}.${filter[1]}`]: [filter[2], filter[3]]
			// 		});
			// 	}, {});
			// 	console.log("raw_filters")
			// 	console.log(typeof(raw_filters))
			// 	console.log(raw_filters)
			
			// const filters = JSON.parse(dicts['filters']);
			// console.log(filters)
			// frappe.route_options = route_options
			// console.log(frappe.route_options)
			// var f = {}
			// Object.keys(raw_filters).forEach(function(key) {
			// 	console.log(key, raw_filters[key]);
			// 	f[key]=raw_filters[key]
			// });
			// // frappe.set_route('')
			// console.log("f")
			// console.log(f)
